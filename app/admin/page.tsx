...
const initialMatch: MatchInsert = {
  home_team_id: null,
  away_team_id: null,
  home_score: 0,
  away_score: 0,
...
const initialTeam: TeamInsert = {
  name: '',
  code: '',
  flag_url: null,
  group_name: '',
  fifa_points_before: 0,
  fifa_rank_before: null
};

const initialPlayer: PlayerInsert = {
  firstname: '',
  lastname: '',
  team_id: null,
  position: 'GK',
...
//   const [form, setForm] = useState<FormData>({});
  ...
  const closeDialog = useCallback(() => {
    ...
    setEditing(null);
    if (tab === 'matches') setMatchForm(initialMatch);
    if (tab === 'teams') setTeamForm(initialTeam);
    if (tab === 'players') setPlayerForm(initialPlayer);
    setError('');
    setDialogOpen(true);
  }, [tab, setError]);
          ...
          <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-6">          
              {tab === 'matches' && (
                <MatchFormFields 
                  form={matchForm} 
                  setForm={(val) => setMatchForm(prev => ({ ...prev, ...val }))} 
                  teams={teams} 
                />
              )}
              {tab === 'teams' && (
                <TeamFormFields 
                  form={teamForm} 
                  setForm={(val) => setTeamForm(prev => ({ ...prev, ...val }))} 
                />
              )}
              {tab === 'players' && (
                <PlayerFormFields 
                  form={playerForm} 
                  setForm={(val) => setPlayerForm(prev => ({ ...prev, ...val }))} 
                  teams={teams} 
                />
              )}
            </div>
